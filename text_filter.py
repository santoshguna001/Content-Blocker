...
def filter_text(text):        
    ...
    for i in range(len(bad_words_list_final)):
        text=temp2
        temp=re.findall(r"\b%s\b" %bad_words_list_final[i], text,re.IGNORECASE)
        temp1=list(substring_indexes(bad_words_list_final[i],text.lower()))
        #temp1=re.search(r"\b%s\b" %bad_words_list_final1[i], text,re.IGNORECASE)
        if temp:
            text1=text
            for m in range(len(temp1)):
                #print(temp1[m])
                k=temp1[m]+len(bad_words_list_final[i])
    ...
    del i,j,m,temp,temp1,temp2,text1
